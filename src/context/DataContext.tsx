...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const analyzeErrors = async () => {
    if (systemErrors.length === 0) return "Ошибок не обнаружено. Система работает стабильно.";
    
    const errorLog = systemErrors.map(e => `[${e.createdAt}] ${e.route}: ${e.message}`).join('\n');
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Проанализируй следующие системные ошибки и предложи решения:\n${errorLog}`,
        config: {
          systemInstruction: "Ты - ИИ помощник системы Uzbechka. Твоя задача - анализировать логи ошибок и давать краткие, технически точные советы по их исправлению."
        }
      });
