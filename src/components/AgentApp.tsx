...
export const AgentApp: React.FC = () => {
  const { products, categories, orders, stats, users, createOrder, updateOrder, updateUser, speak, isOnline } = useData();
                    ...
                    <div key={product.id} className="flex items-center justify-between p-3 bg-stone-50 rounded-xl border border-stone-100">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-uzum-muted">{(product.discountPrice || product.price).toLocaleString()} сум</p>
                        </div>
                      </div>
                      <button 
                    ...
                    {agentCart.map(item => (
                      <div key={item.id} className="flex justify-between items-center text-sm">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="font-bold">{(item.price * item.quantity).toLocaleString()} сум</span>
