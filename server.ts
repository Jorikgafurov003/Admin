...
try {
  ...
  if (settingsCount.count === 0) {
    const defaultSettings = [
      ...
      { key: 'uzum_nasiya_card', value: '4444 0000 0000 0000' },
      { key: 'admin_card', value: '8600 1234 5678 9012' }
    ];
...
async function startServer() {
  ...
  const logError = (message: string, route: string, stack?: string) => {
    try {
      db.prepare("INSERT INTO system_errors (message, route, stack) VALUES (?, ?, ?)").run(message, route, stack || '');
    } catch (e) {
  ...
  app.use((req, res, next) => {
    ...
    res.on('finish', () => {
      ...
      if (duration > 1500) { // Log slow API calls
        logError(`Slow API call: ${duration}ms`, req.url);
      }
    });
    next();
  });
  ...
  app.patch("/api/orders/:id", (req, res) => {
    const allowedFields = ['clientId', 'agentId', 'courierId', 'totalPrice', 'paymentType', 'paymentStatus', 'collectionStatus', 'orderStatus', 'location', 'latitude', 'longitude', 'deliveryPhoto'];
  ...
  app.get("/api/system-errors", (req, res) => {
    const errors = db.prepare("SELECT * FROM system_errors WHERE fixed = 0 ORDER BY createdAt DESC LIMIT 50").all();
    res.json(errors);
  });

  app.patch("/api/system-errors/:id", (req, res) => {
    db.prepare("UPDATE system_errors SET fixed = ? WHERE id = ?").run(req.body.fixed ? 1 : 0, req.params.id);
    res.json({ success: true });
  });

  app.delete("/api/system-errors", (req, res) => {
    db.prepare("DELETE FROM system_errors WHERE fixed = 1").run();
    res.json({ success: true });
  });
